...
serverattr = ts.server.ServerService
...
for server in servers:
   ...
   for v in vo:
		print '<tr><td><font face="Georgia" size="2">%s</td><td><font face="Georgia" size="2">%s</td><td><font face="Georgia" size="2">%s</td><td><font face="Georgia" size="2">%s</td><td><font face="Georgia" size="2">%s</td></tr>' % (v.packageName,v.name,v.type,v.version,v.status)
...
file3.close
#serverRef = com.opsware.server.ServerRef(33810004)
#for server in servers:
#   print server
#   vd = ts.server.ServerService.getCustAttr()
#   for d in vd:
#		print d
...
<td>
			<br>
			<hr>
			<a href="/cgi-bin/import_data.cgi?file=server.sql&file=sa_installed_software.sql&file=sa_interfaces.sql">Import data to AIR</a>
			<br>
			<hr>
			</td>
<tr>
